...
export function validatePlanExecution({ sourceTask, plan, thread, provider }) {
  ...
  if (!thread || !['codex', 'claude'].includes(provider)) {
    throw new Error('Choose a connected Relay to execute the reviewed plan.');
  }
  if (!text(thread.cwd) || !text(sourceTask.repo_path)) {
    throw new Error('The reviewed plan and target Relay must identify their workspace.');
  }
  if (resolve(thread.cwd || '') !== resolve(sourceTask.repo_path || '')) {
    throw new Error('The reviewed plan can only run on a Relay in the same workspace.');
  }
