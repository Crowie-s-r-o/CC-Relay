...
export class PlanRunCoordinator {
   ...
   * steps that were queued or running when Relay died, and their rows would otherwise stay
    ...
    if (isRunActive(run.status)) {
      for (const step of readySteps(steps)) {
        ...
        if (outcome.task) {
          ...
          // A transient enqueue refusal (a terminal closing, Relay stopping) must not fail
  ...
  enqueueStep(run, step) {
    ...
    // A task may already exist for this step when Relay died between the enqueue and the
