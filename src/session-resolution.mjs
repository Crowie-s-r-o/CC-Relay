// Which provider registry owns the primary session a submission names. Plan council's primary
// thread is its Codex reviewer; the server resolves its separate authorThreadId through the
// Claude registry after this first lookup.
...
export const SESSION_NEVER_SEEN = {
  claude: 'Relay has never seen that Claude Code session. Refresh the session list.',
  codex: 'Relay has never seen that terminal on its shared Codex server. Refresh the session list.',
};
...
// Only a session Relay has never seen at all is rejected, because then there is genuinely
