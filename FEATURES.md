...
CC Relay converts raw provider events into a readable activity stream containing commands, tool calls, file changes, messages, errors, and final results. The filter rail shows live counts for All, Highlights, Commands, Conversation, My messages, and AI messages. All includes provider-exposed Codex reasoning summaries and OpenCode reasoning blocks. There is no separate Thinking toggle. Conversation keeps both speakers together in chronological order. My messages includes the canonical original request plus accepted updates, while AI messages includes only actual Codex, Claude, or OpenCode response text and excludes provider status notices. Filtered rows keep their original signal numbers, and tool rows show elapsed time when the provider reports it. Signal counts, the status bar, and Copy log all follow the same visible set. Task Activity text can also be selected directly; live refreshes preserve the range, and the desktop right-click menu exposes native Copy. Prompt Copy writes only the user-authored prompt bodies, without generated numbering or an Original request label.

Task views default to **Original terminal** inside CC Relay. On macOS, the view reads the exact owned CLI terminal screen without foregrounding Terminal.app. **Show original terminal** expands the screen in Relay's own dialog. The screen is read-only; the same task continuation composer sends follow-ups or live updates, with drafts and attachments preserved across views. Screen refresh pauses when the view is hidden and preserves text selection and manual scroll. Council and Turbo tasks offer their own verified terminal choices. **Relay activity**, **Conversation**, **My messages**, and provider-derived AI messages remain available, and both view preferences are remembered. Headless runs, closed or unverifiable windows, and unsupported platforms display the activity fallback without launching another session. See [Original Terminal Default](wiki/original-terminal-default.md) and [Terminal Window](wiki/terminal-window.md).
