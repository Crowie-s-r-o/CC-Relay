...
Task views default to **Original terminal**. Selecting a task or this view opens its verified native CLI window on macOS or Windows, with the CLI's own keyboard shortcuts, menus, and input. Background refreshes never reopen it. **Relay activity**, **Conversation**, **My messages**, and provider-derived AI messages keep the current event view available, including its continuation composer. The inline mode and expanded-window view are remembered. Headless runs, closed windows, and unverifiable terminal identities display the activity fallback without creating another session. OpenCode and Windows Claude execution currently use this fallback. See [Original Terminal Default](wiki/original-terminal-default.md) and [Terminal Window](wiki/terminal-window.md).
