...
- Maximize independence. Two steps that touch different files and do not read each other's output must not depend on each other, because CC Relay runs independent steps at the same time in different sessions.
 ...
 *
 * Contract v2 adds an explicit dependency graph. CC Relay executes a breakdown as a
...
export function buildBreakdownPrompt({ plan = {}, guidance } = {}) {
  ...
  return `You are helping break a saved implementation plan into a dependency-aware set of well-scoped tasks that CC Relay can execute for AI coding agents. Work in read-only planning mode: you may inspect the repository, but do not edit files, run implementation commands, or delegate work.
