...
Before a conversation is registered, the task can attach to a temporary `launch:<uuid>` target. That target requires the exact persisted task ID, provider, project, live owned launch, and queued/running state. It expires when the conversation binds, and the view reconnects to the actual persisted conversation. This exposes the CLI's own trust and login screens. The embedded binding window is 60 seconds; a timeout remains an execution failure, not permission to type into an arbitrary terminal.
...
- OpenCode's current runner and any Claude headless fallback have no original interactive TUI. They report that explicitly. The operator can choose Relay activity and message filters, but those views are never silently substituted for an original terminal.
- Every different selected task starts on Original terminal. An explicit view choice remains in effect during refreshes of that same task. Old saved activity preferences cannot override a new task's default.
