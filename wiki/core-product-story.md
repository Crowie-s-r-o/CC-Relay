...
6. Forward-planning Turbo, where every prompt gets a fresh stronger planner followed by one fresh lower-cost executor that owns the complete graph and may use internal sub-agents.
...
- Finished direct tasks and completed Turbo executors keep their provider conversation ID. **Continue session** opens a replacement terminal and resumes that same conversation under the same task row. See [[same-task-session-continuation]] and [[turbo-execution]].
...
- Plan council retains its provider-order contract. Turbo retains its selected planning and execution models while using one fresh terminal per stage and queue-level executor concurrency. See [[plan-council]] and [[turbo-execution]].
