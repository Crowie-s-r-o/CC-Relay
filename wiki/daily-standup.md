---
name: Daily Standup
description: How CC Relay uses a date-gated isolated AI run to synthesize saved conversations into separate task and blocker sections.
type: architecture
tags:
  - relay
  - standup
...
The Task queue heading exposes **Standup** beside the Queue and History controls. Opening the modal does not start AI. The date is intentionally blank so the user can first choose **Short**, **Standard**, or **Detailed**, then select a local calendar day. Selecting the date starts generation.

CC Relay asks the currently selected composer provider to synthesize saved prompts, assistant responses, final results, and errors. The result is not a mechanical task listing. The AI groups related tasks, retries, and follow-ups, explains what changed and how, and classifies every item as a completed **Task** or unresolved **Blocker**.
...
- A refreshed renderer requires both `capabilities.aiStandupGeneration` and `capabilities.aiStandupConfiguration`. This prevents an older active backend from silently ignoring the selected length or structured output contract.
...
- Every item is capped at 1,200 characters and the complete normalized output at 12,000 characters.
...
Opening Standup clears the date and performs no provider call. The user chooses length first if needed, then selecting a date generates that day. The modal provides:
...
```json
{
  "standup": "- Task: Added date-gated generation with selectable output length.",
  "copyText": "Tasks\nAdded date-gated generation with selectable output length.\n\nBlockers\nNone",
  "tasks": [
    "Added date-gated generation with selectable output length."
  ],
  "blockers": [],
...
The endpoint validates the pinned project, optional Relay id length, provider, length enum, local-day interval, eligible source tasks, provider readiness, and global generation slot before returning normalized structured output.
...
Focused tests cover local dates, DST day lengths, exact project and Relay filtering, status filtering, prompt and response history, source bounds, prompt-injection framing, length validation and limits, task and blocker normalization, prefix-free clipboard text, date-gated UI wiring, mixed-version capability gating, Codex and Claude process flags, timeouts, provider fallback, and concurrency. Real one-shot smoke checks passed with Codex CLI 0.146.0 and Claude Code 2.1.84 using the new classified contract. The complete 810-test repository suite passes.
