...
- **Automatic pools cap concurrency at nineteen (updated September 5).** The current pipeline requires capacity for `workerCount` execution lanes plus one planning lane, and `validateInstanceLimit` caps a project at twenty instances per provider. In pool mode the field advertises `max="19"` and the renderer clamps state to it. Legacy live-terminal Turbo keeps eight workers for stored compatibility.
