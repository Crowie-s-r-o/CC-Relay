...
| Gap risk | Amber | Claude authentication is healthy again, but the running CC Relay process predates the new resume, artifact, and execution routes and has active tasks. Restarting it for a successful new-backend smoke test would interrupt user work. |
...
9. **Execute on CC Relay** validates a completed source, selected provider, live session, workspace, model, effort, and attachments, then creates a linked direct Execute task containing the original request and final plan.

### Top 3 Risks

1. A successful live pass still requires a normal CC Relay restart after its active tasks finish. The current process serves the old runner and cannot hot-load backend routes.
...
1. After a normal CC Relay restart, run one small live council and execute its result on a second CC Relay as a release smoke test.
...
**Ship with mitigations.** The implementation and deterministic coverage are strong. Claude authentication is healthy again. Before calling the new workflow live-verified, restart CC Relay after active work finishes and complete the smoke test above.
...
- Fixed: completed plans had no API or UI path to execute on a chosen CC Relay.
...
- Execution is restricted to the source workspace. A CC Relay in another project is intentionally not an eligible target.
...
- The OAuth failure was reproduced directly and authentication was later restored, but no successful pass used the new runner because the live CC Relay backend is still the older process and has active tasks.
...
- Execution is an explicit user choice and inherits the selected CC Relay's settings.
...
The execution capability existed in source but was too implicit, and the live CC Relay process still lacked `planArtifacts` and `planExecution`. The completed-plan UI now makes both deliverables explicit in one place. It always shows the local `plan.md` path, explains the restart boundary on an old backend, lists every opened same-workspace Codex and Claude CC Relay, and names the chosen provider and CC Relay on the execution button.
...
| Gap risk (edge cases, error handling, completeness) | Green | No opened CC Relay, old backend capability, disconnected selection, Claude sign-out, duplicate clicks, cross-workspace selection, missing attachments, and stale provider sessions all fail visibly or are rejected server-side. |
...
1. The new backend routes are not active until the current CC Relay process restarts. Static UI correctly explains this, but the execute click cannot succeed before restart.
...
1. Restart CC Relay after active work finishes, select task 194, choose one Codex and one Claude session in turn, and execute a harmless reviewed-plan fixture as the final smoke test.
...
- Fixed: plan execution depended on the global CC Relay selection and was easy to miss in the generic task action row.
...
- Multiple completed plans may remember different CC Relay selections in memory. This is intentional and resets with the renderer process.
- A new CC Relay can appear during polling. The selector refreshes from current thread discovery and preserves an existing valid choice.
...
- The selector chooses the first eligible CC Relay when no current or remembered CC Relay remains. The button names that fallback before any request is sent.
