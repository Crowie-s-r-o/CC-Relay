...
## Inline release brief

GitHub release notes now travel with the update state on both discovery paths. `electron-updater` supplies the latest release's Atom HTML to automatic builds, while the fixed GitHub API checker supplies the Markdown release body to Windows portable builds. `src/desktop-release-notes.mjs` reduces either format to at most 20 categorized plain-text items of at most 240 characters each. It recognizes the release contract's Added, Changed, Fixed, and Security sections, deduplicates items, removes links and markup, and selects only the requested version when `electron-updater` returns a multi-version array.

> [!important]
> GitHub release content is untrusted renderer input. The coordinator normalizes it before publication, the status boundary normalizes it again, and `public/app.js` creates each row with `textContent`. Do not replace this path with release-body HTML interpolation.

The dialog uses the existing version route as its only expressive element. A compact ledger below progress shows category chips and release facts, with its own bounded scroll region. An unchanged-content signature prevents status refreshes from rebuilding that ledger, preserving release-note scroll position and text selection. The complete card also gives its body a scroll fallback so small desktop viewports keep the header and actions reachable. Missing summaries degrade to a short prompt to open the official release. The former five-minute cadence strip was removed because it described an implementation detail rather than helping the operator decide what to do.

> [!note]
> Verification on August 21, 2026: all 43 focused release discovery, updater, status,
> presentation, and startup checks pass. All 1,651 repository tests pass,
> `npm run release:check` confirms v0.2.20 metadata, JavaScript syntax checks pass,
> `git diff --check` is clean, and the added-line audit contains no em dash characters.
...
> [!important]
> The native Crowie application icon starts from `build/icon.png`. `win.icon` points at that PNG and electron-builder converts it for the Windows executables. `mac.icon` points at `build/icon.icns`, an artifact this repository tracks and regenerates with `npm run icon:mac`, because the builder's own PNG-to-icns derivation writes non-conforming retina slots. electron-builder copies a supplied icns verbatim and validates nothing about it, so `test/mac-icon-geometry.test.mjs` is that file's only gate. See [[macos-app-icon]]. `public/favicon.svg` controls only the renderer tab icon and cannot replace the Electron logo in the Dock, Finder, taskbar, or installed application.
...
- `build/icon.png`: 1024px transparent Crowie source used for the Windows icon and the development Dock icon.
- `build/icon.icns`: tracked specification-conforming macOS bundle icon, regenerated by `scripts/generate-mac-icon.mjs` and copied verbatim into the bundle from `buildResourcesDir`, never through the asar `files:` payload.
...
- **The tag is pushed but the Releases page is empty:** expected publicly while the hosted workflow and local deploy are still assembling the draft. If deploy has stopped, inspect the exact workflow run and the local deploy error it named. A failed build leaves no usable handoff. Do not move the tag. After repairing release orchestration on `main`, run `npm run deploy -- --recover-only`; deploy can dispatch the current validated workflow while every build and release step still checks out the original tag. See [[open-source-releases]].
