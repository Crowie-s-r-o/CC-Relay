...
The right side of the global header uses the space formerly occupied by **Pause queue** for a compact five-meter subscription runway. Its visible labels are **Cla 5h**, **Cla Week**, **Fable**, **Cod 5h**, and **Cod Week**, while the accessible progress-bar labels keep the full provider and window names. Below 50 percent is green, 50 through 74 percent is yellow, 75 through 89 percent is orange, and 90 percent or more is red. Percentage text and the semantic state keep the reading independent of color. Fable is always independent from the all-model Claude week: a directly reported zero shows `0%` with no countdown when Claude supplies no reset, while a missing Fable section shows `--`. A provider-declared stale frame retains every value from the previous successful sample and marks it last known, so an older cached weekly percentage cannot replace a newer one. Hover titles expose corresponding reset times and stale state. Remaining-time countdowns use larger 9px monospace text for readability. The two five-hour windows show hours and minutes, while weekly windows with a reported reset show days and hours.
