...
> [!important]
> Current backends advertise `capabilities.disposableTerminalPools`. In that mode the composer shows per-project Codex, Claude, and OpenCode maximum instance controls instead of a live terminal picker. References below to a selected CC Relay, immediate finished-task follow-up, manual assignment, or the parallel batch bar describe the legacy persistent-task compatibility path unless stated otherwise. See [[disposable-terminal-pools]].
...
The center of the application header is a global monitoring rail, not a connection, queue, or generic status summary. It contains every task whose persisted status is `running` plus every valid manual Terminal session mode task whose status is `open`, including work from every project. Running tasks sort first. An open manual session remains between turns until the operator uses **Complete session** or its retained terminal is confirmed closed. Queue and History remain scoped to the selected Launchpad; this monitor is the intentional cross-project exception.

Each compact card shows the task number, project, provider session, live or open duration, prompt, latest recorded agent response, and native token speed when available. The project name leads the prompt row in its identity color and owns a protected minimum width, so state, throughput, and duration metadata cannot squeeze it out, including when the monitor is at the bottom. The task name uses the remaining width. `src/running-task-feed.mjs` selects only Codex `agentMessage`, Claude `claude/message`, and OpenCode `opencode/message` events, so command output, tool protocol, and hidden reasoning never replace the response preview. It separately folds the newest native cumulative token-usage event. A running task without an agent message says **Waiting for the first agent response**. An open session falls back to its latest result or error and otherwise says that it is ready for another command. Clicking a card selects its pinned project when available, returns to Queue, and opens the task's complete Task Activity view.
...
The right side of the global header uses the space formerly occupied by **Pause queue** for a compact five-meter subscription runway. Its visible labels are **Cla 5h**, **Cla Week**, **Fable**, **Cod 5h**, and **Cod Week**, while the accessible progress-bar labels keep the full provider and window names. Below 50 percent is green, 50 through 74 percent is yellow, 75 through 89 percent is orange, and 90 percent or more is red. Percentage text and the semantic state keep the reading independent of color. When a successful Claude response reports no distinct Fable allowance, the Fable meter uses the all-model weekly percentage and reset as a marked shared fallback instead of showing `--` as though the model were unavailable. An explicit model-breakdown refresh failure retains a prior real Fable value as last known, or shows `--` when no real value exists. Hover titles expose the corresponding reset time, explain the shared fallback, and identify a retained value as last known. Remaining-time countdowns use larger 9px monospace text for readability. The two five-hour windows show hours and minutes, while the three weekly windows show days and hours.
...
2. A filter toolbar (All, Highlights, Commands, Conversation, My messages, AI messages) with an independent **Thinking** switch and Copy log
...
The task header is intentionally dense: reduced inspector padding, a 20px task title, a complete task-definition preview clamped only after three wrapped rows, inline compact metadata, a smaller action button, and tighter Prompt and Result sections. Prompt-derived definitions must reach CSS unshortened so available header width determines the visible text. The execution terminal should receive as much vertical room as possible without hiding task context.
...
Prompts and Result use compact evidence disclosures inside the full task detail modal. The modal also owns attachments, session conversation, Turbo graph, and every Plan council stage and artifact. Prompts contains the original request plus every accepted follow-up in order and opens whenever the full task record opens. It remains manually collapsible until the dialog is closed. Its summary reports the prompt count and latest prompt preview. Result retains the latest outcome. See [[task-detail-modal-and-app-zoom]].
...
The Turbo concurrent execution count is the deliberate exception. It is a number field, not a picker, so a per-keystroke handler that clamped it into range made a two-digit count impossible to type. It commits on `change`, guards Return so an implicit form submission cannot queue the prompt, and resyncs on `blur`. Its compatibility ID remains `#turbo-worker-count`. See [[turbo-execution]].
...
On a current backend, the composer shows Codex, Claude, and OpenCode provider tabs plus per-project maximum instance inputs. It shows each provider's active count and does not require a live CC Relay selection. A fresh task waits for capacity and launches the native terminal or headless process required by its workflow. The selected project's **Keep task terminals open** choice applies to Codex and Claude final launches; OpenCode has no native terminal window to retain. **Settings** opens the native dialog for that project's launch layout and diagnostics. The Launchpad has one **Add project** action that pins and selects a folder without opening a terminal. Manual **Launch Codex** and **Launch Claude** actions for interactive sessions remain in the terminal panel.
...
CC Relay cards derive their visible activity from task ownership rather than relying only on the provider thread title. A running direct task shows its task number and prompt, current Turbo terminals identify planner or executor role, an idle terminal with assigned work shows its waiting count and next task, and a free terminal says it is ready. The state badge follows this derived running, queued, or idle state.

Turbo queue cards retain the canonical `queued` or `running` status badge and add a compact secondary forward-plan stage badge. **Forward plan** means a queued Turbo parent has no reusable graph yet, **Planning ahead** means its queued graph is currently being prepared, **Plan ready** means a validated graph will bypass planning at execution time, and **Executor running** means one fresh session owns the complete graph. Complete and failed variants are terminal signals only. The stage badge is width-constrained so it cannot expand the task card.

Terminal activity follows the persisted lifecycle: a fresh planner preparing a queued parent reads **Planning ahead · Task #n**, and the later fresh execution terminal reads **Turbo executor · Task #n**. The planner normally closes when planning finishes, so it cannot become an idle reusable worker.
...
## Turbo stages and dispatch graph

Current automatic Turbo task cards keep their canonical task status and forward-plan marker, then add one compact two-stage manifest. It reads **Planning**, the selected model and effort, and **fresh session**, followed by **Execution**, its model and effort, **one terminal per Turbo prompt**, and the queue concurrency. Historical persistent Turbo cards retain their planner and worker identity manifest for compatibility. The renderer never invents a CC Relay number from an old worker slot or title.
...
The Turbo composer keeps the optional Plan council as one compact route. Its unchecked **Use Plan council for this prompt** option sits beside a keyboard-accessible question-mark button. The disclosure explains that the selected first provider creates the JSON graph, the second checks and corrects it, execution waits, the pass adds time, and Claude CLI sign-in is required. Since August 12, 2026 the disclosure also carries the Turbo supporting sentence and the planner node description, which no longer print in the panel.

> [!note]
> Execute and Forward-planning Turbo wrap their optional Plan council controls in the same compact neutral review surface: a 12px radius, quiet gray-blue border, white toggle row, and a pill-shaped provider-count badge. Checked state uses one soft blue focus treatment instead of nested blue outlines. Keep provider identity colors inside the expanded route only. Execute prints the **Optional review / Plan council** eyebrow-plus-title heading above that surface. Turbo does not: its council surface sits under the single **Planning and execution** header row, which also carries the `1 planner` or `2 providers` chip and the readiness chip.
...
When enabled, both workflows use the same rounded provider nodes, 3px provider accent rails, numbered **Author** and **Reviewer** roles, agent icons, rounded model and effort controls, and central review handoff. The handoff is a thin line with one circular arrow marker, which is the route's signature visual and makes the execution sequence readable without extra decoration. Execute adds its compact Claude revision strip and readiness pills. Turbo adds only its **Codex first / Claude first** segmented control and swaps node order and role copy. Codex keeps its selected planner model and effort; Claude uses its catalog-backed model and normalized effort. Execution settings remain separate. Disabled routes are hidden in both workflows, and the expanded route collapses to one column on narrow screens.

The switch is unchecked by default, so disabled Turbo keeps its planner-to-executor behavior and does not require Claude availability. Enabling it adds one quality pass and latency before execution starts. Reviewer readiness uses the selected Claude catalog and signed-in CLI state. Execute and Turbo keep independent per-prompt council switches.

Turbo task metadata states the selected provider order before execution. During `planning` and `reviewing`, copy identifies the provider that owns the current stage. A Codex terminal exists only while Codex owns that stage. Queue and detail labels keep **Council review** alongside the canonical task badge.
...
> session that was in fact connected. Plan council readiness and the Turbo concurrency count are
