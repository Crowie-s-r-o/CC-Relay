...
CC Relay launches owned Codex terminals with `--dangerously-bypass-approvals-and-sandbox`. Normal Execute work must therefore receive full filesystem access, while Plan council reviews and Turbo planning must remain deliberately read-only.

## Failure

Codex app-server treats `turn/start.sandboxPolicy` as an override for the current turn and subsequent turns. CC Relay previously sent an explicit `readOnly` policy for planning, but omitted `sandboxPolicy` for normal Execute work. Resuming the thread with the legacy `sandbox: workspace-write` field did not reset the active policy in the observed Codex 0.145.0 session.

The result was sticky read-only state:

1. A CC Relay-owned terminal began with `danger-full-access`.
...
> [!note]
> The full-access value matches the native CC Relay launch command and existing terminal behavior. This change adds no environment variable and does not weaken planning stages.
...
The source change requires a normal CC Relay backend restart. Static browser refreshes cannot replace the in-memory Codex app-server client.
