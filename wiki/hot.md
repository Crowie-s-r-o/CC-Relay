...
> [!important]
> **August 12: the header now shows provider subscription runway instead of a queue pause button.** Four compact bars report Claude's five-hour session, all-model weekly, and Fable weekly usage plus Codex weekly usage. They refresh every five minutes, expose reset details, preserve last-known values across temporary failures, and use warning amber at 70 percent and critical red at 90 percent. Provider credentials remain inside the installed CLIs. The backend pause contract remains available to queue integrations. Live Claude and Codex probes succeeded, light and dark layouts stayed overflow-free at 1680, 720, and 420 pixels, all 1,419 tests pass, and release metadata is consistent. See [[provider-usage-monitor]] and [[interface-layout]].
