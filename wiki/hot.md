...
> previously missed that row and copied the all-model weekly percentage and seven-day countdown.
> A later per-model rate-limit frame could then replace a successful `3%` week with cached `2%`.
> Relay now parses standalone Fable values without requiring a reset, never borrows the all-model
> week for Fable, and treats provider-declared stale frames as status-only after a successful
> sample. See [[provider-usage-monitor]] and [[interface-layout]].
...
> without that the app wedges behind an invisible modal. `terminalWindowView`
> joined the app-wide `ui-layout-preferences` record, which `PATCH` replaces in full: always save
> through `uiPreferencesPayload()`.
> A design polish round then folded the toolbar: while docked the whole `.event-toolbar` row is
> hidden and the live `.event-tools` cluster (Thinking, Copy log, Window) MOVES into the dialog header
> slot `#terminal-window-tools`, which is why the dock records a second slot and why the tools are
> restored BEFORE the section on close, so nothing ever measures a section holding a stripped toolbar.
> Hiding that row also dropped it out of grid flow, so the docked section went from five
> `grid-template-rows` to FOUR; the five-row template would have handed the `1fr` to the continuation
> composer and collapsed the scrollback. The same round split hover from pressed on the view rail
> across five channels and moved the light pressed accent to the app's existing `#2239c9` indigo.
> See [[terminal-window]] and [[durable-ui-layout-preferences]].

> [!important]
> **September 3: the macOS bundle icon is now a repository-tracked conforming `icon.icns`.**
> electron-builder 26.15.3 derived an icns whose `ic13` held 512 pixels and `ic14` held 1024, where
> the specification requires 256 and 512. Loaded directly, AppKit then reported fractional logical
> sizes on the 256px and larger representations, and lost @2x pairing at every level,
> 16pt and 32pt included. IconServices was checked and is not affected: it
> normalizes into its own representation set. A survey of the installed applications found only
> CC Relay and the older Relay bundle non-conforming out of well over a hundred PNG-based icns
> bundles. `mac.icon` now points at `build/icon.icns`, regenerated by `npm run icon:mac`, and
> `test/mac-icon-geometry.test.mjs` validates it in pure Node on every platform. electron-builder
> validates a supplied icns not at all, so that test is the only gate on the file. This removes the
> trigger for future builds. It does not repair an already-poisoned macOS icon cache, which stays a
> separate user-side remedy. See [[macos-app-icon]].
