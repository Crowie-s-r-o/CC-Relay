...
test('Electron binds its embedded CC Relay server to an available port', () => {
  ...
  assert.match(main, /const rendererUrl = desktopRendererUrl\(endpoint\.url, process\.platform\);/);
...
test('Electron exposes GitHub updater state to the loopback UI', () => {
  ...
  assert.match(main, /restartAndInstall: async \(prepareToInstall\)/);
  assert.match(main, /await prepareToInstall\(\)/);
  assert.match(main, /desktopUpdater\.prepareToInstall\(\)/);
